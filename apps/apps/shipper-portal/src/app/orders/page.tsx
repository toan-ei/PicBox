...
import { Package, Clock, CheckCircle, AlertCircle, Truck, ChevronRight, Search, Home, List, Wallet, User } from "lucide-react";
...
type OrderStatus = "pending" | "picking" | "delivering" | "delivered" | "failed";
...
const ORDERS: ShipperOrder[] = [
  { id: "PB-20260003", customer: "Lê Văn C", phone: "0923456789", from: "Quận 3", to: "Thủ Đức", cod: 180000, fee: 42000, weight: 2.1, status: "picking", time: "09:30" },
  { id: "PB-20260010", customer: "Mai Thị E", phone: "0934000010", from: "Bình Thạnh", to: "Gò Vấp", cod: 0, fee: 28000, weight: 0.8, status: "pending", time: "10:15" },
  { id: "PB-20260011", customer: "Trung Hiếu", phone: "0934000011", from: "Quận 1", to: "Bình Thạnh", cod: 95000, fee: 32000, weight: 1.2, status: "pending", time: "10:45" },
  { id: "PB-20260001", customer: "Nguyễn Văn A", phone: "0901234567", from: "Quận 1", to: "Quận 7", cod: 0, fee: 35000, weight: 1.5, status: "delivered", time: "08:12" },
  { id: "PB-20260002", customer: "Trần Thị B", phone: "0912345678", from: "Bình Thạnh", to: "Gò Vấp", cod: 250000, fee: 28000, weight: 0.8, status: "failed", time: "09:05" },
];
...
export default function ShipperOrdersPage() {
  ...
  const filtered = ORDERS.filter((o) => {
    ...
    const matchTab =
      tab === "active"  ? ["picking", "delivering"].includes(o.status) :
              ...
              <Link key={order.id} href={`/delivery/${order.id}`}>
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="text-[13px] font-bold text-white">{order.id}</p>
                  ...
                  <div className="flex items-center justify-between" style={{ borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: "0.75rem" }}>
                    <div className="flex items-center gap-3">
                      {order.cod > 0 && (
                        <span className="text-[12px] font-semibold text-emerald-400">COD ₫{order.cod.toLocaleString()}</span>
                      )}
                      <span className="text-[11px] text-slate-600">Cước ₫{order.fee.toLocaleString()}</span>
                    </div>
